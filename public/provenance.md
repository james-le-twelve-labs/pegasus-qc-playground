...
## Check-engineering notes (feeds the tutorial)

Three lessons from tuning the lip-sync check, in the order they cost us time.
All were verified with repeated runs on the same assets — none of these were
one-off sampling noise.

1. **Field descriptions are the prompt.** The first definition ("True if lips
   articulate the words that are heard") passed a **closed-mouth clip with TTS
   over it**. Rewording to force visual attention ("True ONLY if you can SEE
   the mouth repeatedly open and close…", plus an explicit
   `mouth_visibly_articulating` field) made the same model catch it.
2. **Conditional checks need an applicability gate.** On speech-less clips
   (product shot, silent card shuffle, a dog on a beach) the model returned a
   whole-clip segment with every boolean false and a "no speech audible" note —
   which naive pass/fail logic renders as a lip-sync FAIL on a clip with no
   lips. The `speech_audible` field gates it: judge `lips_match_words` only
   where speech is actually audible.
3. **Field order matters.** Placing `speech_audible` FIRST made the
   TTS-over-still-mouth clip *pass again*, consistently: once the model
   answered "speech is audible," the mouth booleans followed the
   speaking-person prior instead of the pixels. Moving visual fields first and
   wording the gate as audio-track-only ("regardless of whether anyone on
   camera appears to be speaking") restored the catch, consistently. Visual
   judgments before audio anchors; every field states its modality.

The final definition lives in `checks/checks.json` and the public prompt pack.
